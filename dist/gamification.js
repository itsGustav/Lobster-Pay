...
function getDefaultPlayerStats(address) {
 ...
 */
function recordActivity(address, type, amount, metadata) {
    ...
    if (type === 'tip')
        pointsEarned = Math.floor(pointsEarned * 1.5); // Bonus for tips
    if (type === 'escrow')
        pointsEarned = Math.floor(pointsEarned * 2); // Bonus for escrow
    ...
    // First transaction
    if (player.totalTransactions === 1)
        checkBadge('first_tx');
    // Volume badges
    const vol = parseFloat(player.totalVolume);
    if (vol >= 100)
        checkBadge('volume_100');
    if (vol >= 1000)
        checkBadge('volume_1k');
    if (vol >= 10000)
        checkBadge('volume_10k');
    if (vol >= 100000)
        checkBadge('volume_100k');
    if (vol >= 1000000)
        checkBadge('volume_1m');
    // Transaction count badges
    if (player.totalTransactions >= 10)
        checkBadge('tx_10');
    if (player.totalTransactions >= 50)
        checkBadge('tx_50');
    if (player.totalTransactions >= 100)
        checkBadge('tx_100');
    if (player.totalTransactions >= 500)
        checkBadge('tx_500');
    // Streak badges
    if (player.currentStreak >= 7)
        checkBadge('streak_7');
    if (player.currentStreak >= 30)
        checkBadge('streak_30');
    if (player.currentStreak >= 90)
        checkBadge('streak_90');
    if (player.currentStreak >= 365)
        checkBadge('streak_365');
    // Special badges
    if (metadata?.recipients && metadata.recipients >= 10)
        checkBadge('split_master');
    if (metadata?.isTip && amountNum >= 100)
        checkBadge('big_tip');
    saveGameData(data);
 ...
 * Get leaderboard
 */
function getLeaderboard(type = 'volume', limit = 10) {
    const data = loadGameData();
...
function formatProfile(player) {
    ...
    if (player.badges.length > 0) {
        const grouped = { diamond: [], platinum: [], gold: [], silver: [], bronze: [] };
        player.badges.forEach(b => grouped[b.tier].push(b));
        for (const tier of ['diamond', 'platinum', 'gold', 'silver', 'bronze']) {
